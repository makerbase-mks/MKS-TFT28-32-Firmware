...
)
{
  ...
  //W25QXX_Init();
	...
	else 
	{
		...
		{
			i = li + (hi - li) / 2;	
			//SPI_FLASH_BufferRead((u8*)&t,ftinfo.ugbkaddr+i*4+gbk2uni_offset,4);//读出4个字节  
      //W25QXX_Init();
			SPI_FLASH_BufferRead((u8*)&t,UNIGBK_FLASH_ADDR+i*4+gbk2uni_offset,4);//读出4个字节  
			if (src == t[0]) break;
			if (src > t[0])li = i;  
			else hi = i;    
		}
