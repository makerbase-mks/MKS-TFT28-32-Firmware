...
{
	uint8_t retChar;
	if(readUsartFifo(&WifiRxFifo, &retChar, 1) == 1)
		return retChar;
	else
		return 0;
...
EspUploadResult readPacket(uint8_t op, uint32_t *valp, size_t *bodyLen, uint32_t msTimeout)
{
	...
	*bodyLen = 0;
	...
	while (state != done)
	{
		...
		if (getTickDiff(startTime, getTick()) > msTimeout)
		{
			return(timeout);
		}
...
void upload_spin()
{
	...
	case resetting:
		...
		//	uploadPort_close();
			...
			esp_upload.lastAttemptTime = esp_upload.lastResetTime = getTick();
	...
	case connecting:
		if ((getTickDiff(esp_upload.lastAttemptTime, getTick()) >= connectAttemptInterval) && (getTickDiff(esp_upload.lastResetTime, getTick()) >= 500))
		{
			// Attempt to establish a connection to the ESP8266.
			EspUploadResult res = Sync(5000);
			esp_upload.lastAttemptTime = getTick();
			if (res == success)
			{
...
//				//MessageF(" success on attempt %d\n", (connectAttemptNumber % retriesPerBaudRate) + 1);
	...
	case erasing:
		if (getTickDiff(esp_upload.lastAttemptTime, getTick()) >= blockWriteInterval)
		{
			uint32_t eraseSize;
	...
	case uploading:
		...
		if (getTickDiff(esp_upload.lastAttemptTime, getTick()) >= 15)
		{
...
void ResetWiFiForUpload(int begin_or_end)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;//PD9先作为输入，升级时候作为输出
	...
	if(begin_or_end == 0)
	{
		GPIO_ResetBits(GPIOD, GPIO_Pin_9); //update mode
	}
	else
	{
		GPIO_SetBits(GPIOD, GPIO_Pin_9);  //boot mode	
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;//PF2先作为输入，升级时候作为输出
