...
volatile unsigned char mksBpAlrmEn0=0;
volatile unsigned char mksBpAlrmEn1=0;
volatile unsigned char mksBpAlrmEn4=0;
volatile unsigned char mksBpAlrmEn5=0;
	...
	unsigned char breakpoint_homeXY()
	{
		...
		if(RePrintData.saveEnable == 0)
		{
			...
			if(gCfgItems.pwd_reprint_flg==1)
			{
				...
				DecStr2Float(z_breakpoint,&gCfgItems.z_current_position);
		...
		return 0;
#endif
	z_pauseBak[0] = RePrintData.mov_z_coordinate[0];
	for(i=1;i<(COORDINATELENGTH-1);i++)
		z_pauseBak[i] = RePrintData.mov_z_coordinate[i+1];
	//计算Z轴当前位置
	if(gCfgItems.pwd_reprint_flg==1)
	{
		i=0;
...
//	while(checkFIFO(&gcodeTxFIFO) == fifo_full);
	...
	memset(homeGcode_tmp0,'0',sizeof(homeGcode_tmp0));
	sprintf(homeGcode_tmp0,"G1 Z%d\n",gCfgItems.Pause_ZADD);
	pushFIFO(&gcodeCmdTxFIFO,homeGcode_tmp0);	
	pushFIFO(&gcodeCmdTxFIFO,homeGcode4);
	if(gCfgItems.Pause_XPOS != -1)
	{
		memset(homeGcode_tmp0,'0',sizeof(homeGcode_tmp0));
		sprintf(homeGcode_tmp0,"G1 X%d\n",gCfgItems.Pause_XPOS);
		pushFIFO(&gcodeCmdTxFIFO,homeGcode_tmp0);
		positionSaveFlag = 1;
	}
...
void relocalZ(void)
{
	...
	unsigned char homeGcode4[4]="G90\n";	
	unsigned char homeGcode5[7]="G1 E5\n";
...
//	return;
	...
	pushFIFO(&gcodeCmdTxFIFO,homeGcode1);
	sprintf(homeGcode2,"G1 Z-%d F3000\n",gCfgItems.Pause_ZADD);
	//strcat(homeGcode2,homeGcode_tmp1);
	//strcat(homeGcode2,"\n");
	pushFIFO(&gcodeCmdTxFIFO,homeGcode2);
...
{
	unsigned char tmpGcode[FIFO_SIZE-20];
	unsigned char *p;
	unsigned char *q;
	...
	*p++ = 'G';*p++ = '1';*p++ = ' ';*p++ = 'F';*p++ = '3';*p++ = '0';*p++ = '0';*p++ = '0';*p++ = ' ';
