...
*/
	RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOD, ENABLE );	 //PORTA时钟使能 

  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;//PA2.3.4 推挽 	n_3|GPIO_Pin_4;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP ;   
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOD, &GPIO_InitStructure);
	GPIO_SetBits(GPIOD,GPIO_Pin_11);//PA2.3.4上拉 
...
u8 SD_WaitReady(void)
{
	...
	do
	{
		...
		t++;		  	
	}while(t<0XFFFFFF);//while(t<200);//等待 
	return 1;
}
//等待SD卡回应
//Response:要得到的回应值
//返回值:0,成功得到了该回应值
//    其他,得到回应值失败
u8 SD_GetResponse_skyblue(u8 Response)
{
...
//      u8 crc   crc校验值	   
//返回值:SD卡返回的响应															  
u8 SD_SendCmd_skyblue(u8 cmd, u32 arg, u8 crc)
{
    u8 r1;	
	u8 Retry=0; 
	SD_DisSelect();//取消上次片选
...
u8 SD_ReadDisk(u8*buf,u32 sector,u8 cnt)
{
	...
	SD_DisSelect();//取消片选
	//chen 10.6
	if(r1==0)rec_sd.read_disk_err = 0;
	else       rec_sd.read_disk_err = 1;
