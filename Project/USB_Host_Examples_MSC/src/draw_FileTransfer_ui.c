...
static BUTTON_Handle buttonReturn;
...
void draw_dialog_filetransfer(uint8_t WifiTransferDialogType)
{
	...
	else 	if(gCfgItems.language == LANG_COMPLEX_CHINESE)
	{
		if(WifiTransferDialogType == 0)
		{
			GUI_DispStringAt("文件傳輸失敗!", 80, titleHeight  + 75);

		}
		else if(WifiTransferDialogType == 1)
		{
			GUI_DispStringAt("文件傳輸成功!", 80, titleHeight  + 75);
		}
		else if(WifiTransferDialogType == 2)
		{
			GUI_DispStringAt("請檢查是否插入存儲設備!", 40, titleHeight  + 70);
		}
	...
	buttonReturn = BUTTON_CreateEx(0 , 0, LCD_WIDTH / 4 - 2, imgHeight / 2 - 1, hWifiFileDlgWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
	...
	if(gCfgItems.language == LANG_ENGLISH)
	{
		BUTTON_SetText(buttonReturn, "Back");
	}
	else
	{
		BUTTON_SetText(buttonReturn, "返回");
	}	

	BUTTON_SetBkColor(buttonReturn, BUTTON_CI_UNPRESSED, gCfgItems.background_color);
	BUTTON_SetBkColor(buttonReturn, BUTTON_CI_PRESSED, gCfgItems.background_color);
...
void draw_filetansfer(uint8_t file_unzip_flg)
{
	...
	else if(gCfgItems.fileSysType == FILE_SYS_USB)
	{
		...
		else 		if(gCfgItems.language == LANG_COMPLEX_CHINESE)
		{
			if(file_unzip_flg == 1)
			{
				TEXT_SetText(FileTransferText,"文件解壓中");
			}
			else
			{
